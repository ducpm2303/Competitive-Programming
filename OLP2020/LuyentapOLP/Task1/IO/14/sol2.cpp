...
struct node
{
	...
	node* insert(int id, int l, int r, int w);
};

node *null = new node(0, NULL, NULL); //see line 135

node * node::insert(int id, int l, int r, int w)
{
	if(l <= w && w < r)
	{
		...
		if(l+1 == r)
		{
		    //if(id == 5) cout << l << " " << r << " " << this->count << endl;
			return new node(this->count+1, null, null);
		}

		int m = (l+r)>>1;
        //if(id == 5) cout << l << " : " << r << " " << this->count << endl;
		return new node( this->count+1, this->left->insert(id,l, m, w), this->right->insert(id, m, r, w));
	}
...
int main()
{
	...
	rep(i, n)
	{
		...
		root[i] = (i == 0 ? null : root[i-1])->insert( i, 0, maxi, M[a[i]] );
	}
